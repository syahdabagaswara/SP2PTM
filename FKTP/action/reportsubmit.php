...
<?php 
	...
	for($x=0;$x<$datarow;$x++){
	 ...
	 $daps=mysqli_query($koneksi,"SELECT b.id,a.name FROM user a,user b WHERE a.id='$user_id' AND b.id=a.child_id");
	 while($rowps=mysqli_fetch_assoc($daps)){
		$dataps[] = $rowps;
		}
		$idps=$dataps[0]['id'];
		$nameps=$dataps[0]['name'];
		$messagesPS=$nameps." Telah Menyelesaikan Pelaporan";
		
	 $inmessages=mysqli_query($koneksi,"INSERT INTO messages(id,id_user,messages,date,status)VALUES(null,'$idps','$messagesPS','$now','$statusme')");
	 $dadn=mysqli_query($koneksi,"SELECT c.id,a.name FROM user a,user b,user c WHERE a.id='$user_id' AND b.id=a.child_id AND c.id=b.child_id");
	 ...
	 $dnmessages=mysqli_query($koneksi,"INSERT INTO messages(id,id_user,messages,date,status)VALUES(null,'$iddn','$messagesdn','$now','$statusme')");
